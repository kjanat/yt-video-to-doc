...
import { VideoMetadata, Slide, ProcessingResult } from '../types';
...
export class DocumentGenerator {
  ...
  async generateMarkdown(
    ...
    const filename = outputFilename || `${this.sanitizeFilename(videoMetadata.title)}.md`;
  ...
  private buildMarkdownContent(metadata: VideoMetadata, slides: Slide[]): string {
    const lines: string[] = [];
    ...
    slides.forEach((slide, index) => {
      ...
      lines.push('');
      lines.push(`**Time:** ${this.formatTimestamp(slide.startTime)} - ${this.formatTimestamp(slide.endTime)}`);
      lines.push('');
      ...
      // Add frame reference
      const representativeFrame = slide.frames[Math.floor(slide.frames.length / 2)];
      lines.push(`> Frame: ${representativeFrame.frameNumber} at ${this.formatTimestamp(representativeFrame.timestamp)}`);
      lines.push('');
      lines.push('---');
      lines.push('');
    });
