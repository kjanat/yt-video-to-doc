...
import { OCRResult, Slide, Frame } from '../types';
...
export class OCRService {
  ...
  async processSlides(slides: Slide[], onProgress?: (percent: number) => void): Promise<Slide[]> {
    ...
    for (let i = 0; i < slides.length; i++) {
      ...
      const processedImagePath = await this.preprocessImage(representativeFrame.imagePath);
      
      try {
        const ocrResult = await this.extractText(processedImagePath, representativeFrame);
        
        slide.ocrResults.push(ocrResult);
        slide.primaryText = ocrResult.text;
        
        logger.debug(`OCR completed for slide at ${slide.startTime}s: ${ocrResult.text.substring(0, 50)}...`);
      } catch (error) {
  ...
  private async extractText(imagePath: string, frame: Frame): Promise<OCRResult> {
    try {
